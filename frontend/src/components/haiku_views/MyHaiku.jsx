...
import { Details, Share, SharedUsers, ShareIcon, Modify, Toggle, DeleteIcon, MiniTable, TH } from './MyHaiku.styled';
...
const MyHaiku = ({ idx, haiku, users }) => {
  ...
  const renderSharedUsers = () => {
    ...
    return (
      <>
        Shared with <strong>{firstUser}</strong> {userCount !== 0 && <>and <strong>{userCount}</strong> other{userCount > 1 && 's'}</>}
      </>
    )
  }
